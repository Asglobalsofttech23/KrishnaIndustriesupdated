...
import { Grid, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, TextField, Button, Typography } from '@mui/material';
...
const EmpCallNotAttendedLeads = () => {
    ...
    useEffect(() => {
        ...
        axios.get(`${config.apiUrl}/employee/getNotcallattedeByEmpId/${empId}`)
            .then(response => {
    ...
    if (error) return <Typography variant="h6" color="error">Error fetching data: {error}</Typography>;

    return (
        <div>
            <Typography variant="h4" gutterBottom>Leads that don't accept calls</Typography>
